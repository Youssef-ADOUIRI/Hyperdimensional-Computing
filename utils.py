...
def encode_and_save(args):
    ...
    if args.dataset == "mnist":
        trainset = torchvision.datasets.MNIST(
            root=args.raw_data_dir, train=True, download=True, transform=transform
        )
        testset = torchvision.datasets.MNIST(
            root=args.raw_data_dir, train=False, download=True, transform=transform
        )
        ...
        x_train, y_train, x_test, y_test = dataset(
            source=f"./{args.raw_data_dir}/isolet/isolet.pkl"
        )
        x_train, y_train = (
            torch.tensor(quantize(x_train, precision=8)).unsqueeze(1),
            torch.tensor(y_train).long(),
        )
        x_test, y_test = (
            torch.tensor(quantize(x_test, precision=8)).unsqueeze(1),
...
def load(args):
    ...
    print(
        f"Size of encoded training data {train_hd.size()} and test data {test_hd.size()}"
    )
...
def prepare_data(args):
    ...
    trainloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=16, shuffle=True, pin_memory=True, num_workers=1
    )
    testloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=16, shuffle=False, pin_memory=True, num_workers=1
    )
