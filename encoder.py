...
class LinearEncoder:
    ...
    def build_item_mem(self):
        assert (
            self.num > 1
        ), "No need of this function if only one vector in the item memory."
    ...
    def encode_data_extract_labels(self, datast):
        ...
        for i in range(n):
            rv[i] = self.encode_one_img(
                (255 * datast[i][0].view(-1)).int()
            )  # transform to [0, 255]
            labels[i] = datast[i][1]
            if i % 1000 == 999:
...
class RandomFourierEncoder:
    ...
    def GroupRFF(self, x, sigma):
        intervals = (
            ...
            * torch.tensor(
                [
                    scipy.stats.norm.ppf(i * 1.0 / self.gorder)
                    for i in range(1, self.gorder)
                ]
            ).float()
        )
        print(
            "the threshold to discretize fourier features to group elements", intervals
        )
    ...
    def build_item_mem(self):  # create random fourier features for 256 pixel values
        ...
        Cov = np.array(
            [
                np.exp(-correction_factor * self.gamma**2 * ((x - y) / 255.0) ** 2 / 2)
                for y in range(256)
            ]
        )
        k = Cov.shape[0]
        ...
        R = (
            eigen_vectors
            @ np.diag(np.maximum(0, eigen_values) ** 0.5)
            @ eigen_vectors.T
        )
        item_mem = torch.from_numpy(
            np.random.randn(self.output_dim, k) @ R
        ).float()  # this can be reduced for effiency
        self.item_mem = self.GroupRFF(
            item_mem, np.sqrt((R**2).sum(0).max())
        ).T  # compatible for binary HDC (with %2)
    ...
    def encode_one_img(self, x):
        ...
        rv = (
            self.item_mem[x.flatten()]
            .view(bs, channels, num_pixels, -1)
            .transpose(0, 2)
        )
        ...
        for i in range(num_pixels):
            ...
            rv[i] = torch.roll(
                rv[i], shifts=783 - i, dims=-1
            )  # note that this batch shifting might be different from our v1
        rv = torch.sum(
            rv, dim=0
        )  # use sum, natural extends to group bind, result shape: [channels, bs, hyperD]
    ...
    def encode_data_extract_labels(self, datast):
        ...
        data_loader = torch.utils.data.DataLoader(
            datast, batch_size=batch_size, shuffle=False
        )
        for i, batch_img in enumerate(data_loader):
            num_imgs = batch_img[0].size(
                0
            )  # in case the last batch is not equal to batch_size
